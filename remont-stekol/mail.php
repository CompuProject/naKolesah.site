...
$headers .= 'From: <remont-stekol@nakolesah.site>' . "\r\n";


switch ($post_data['modalId']) {
    case 'modal1':
        $subject = "\"Ремонт стекол\" Сообщение с Лендинга - 'Форма заказа обратного звонка'";
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal2':
        $subject = "\"Ремонт стекол\" Сообщение с Лендинга - 'Расчитать точную стоимость'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal3':
        $subject = "\"Ремонт стекол\" Сообщение с Лендинга - 'Подобрать диски'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
        $message .= '<p>Бренд авто: <b>'.$post_data['brand'].'</b></p>';
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal4':
        $subject = "\"Ремонт стекол\" Сообщение с Лендинга - 'Быстрая запись онлайн'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
        mail($to, $subject, $message, $headers);
        $toBd = "Номер телефона клиента:".$post_data['phone']."/n";
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal5':
        $subject = "\"Ремонт стекол\" Сообщение с Лендинга - 'Форма заказа обратного звонка, виджет слева'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
        mail($to, $subject, $message, $headers);
        $toBd = "Номер телефона клиента:".$post_data['phone'];
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal6':
        $subject = "\"Ремонт стекол\" Сообщение с Лендинга - 'Подобрать диски внизу страницы'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
        $message .= '<p>Бренд авто: <b>'.$post_data['brand'].'</b></p>';
        $message .= '<p>Диаметр диска: <b>'.$post_data['diameter'].'</b></p>';
        $message .= '<p>Предполагаемая цена: <b>'.$post_data['price'].'</b></p>';
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal7':
        $subject = "\"Ремонт стекол\" Сообщение с Лендинга - 'Lovec'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
        mail($to, $subject, $message, $headers);
