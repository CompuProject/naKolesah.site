...
$headers .= 'From: <zamena-stekol@nakolesah.site>' . "\r\n";
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal2':
        $subject = "\"Замена стекол\" Сообщение с Лендинга - 'Рассчитать точную стоимость замены'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal3':
        $subject = "\"Замена стекол\" Сообщение с Лендинга - 'Подобрать диски'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
        $message .= '<p>Бренд авто: <b>'.$post_data['brand'].'</b></p>';
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal5':
        $subject = "\"Замена стекол\" Сообщение с Лендинга - 'Форма заказа обратного звонка, виджет слева'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
        mail($to, $subject, $message, $headers);
        $toBd = "Номер телефона клиента:".$post_data['phone'];
...
//        echo json_encode(['callback' => true]);
        break;
    case 'modal6':
        $subject = "\"Замена стекол\" Сообщение с Лендинга - 'Подобрать диски внизу страницы'";
        $message = '<p>Номер телефона клиента: <b>'.$post_data['phone'].'</b></p>';
        $message .= '<p>Бренд авто: <b>'.$post_data['brand'].'</b></p>';
        $message .= '<p>Диаметр диска: <b>'.$post_data['diameter'].'</b></p>';
        $message .= '<p>Предполагаемая цена: <b>'.$post_data['price'].'</b></p>';
